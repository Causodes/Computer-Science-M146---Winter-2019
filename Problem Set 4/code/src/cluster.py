...
class Cluster(object) :
    ...
    def centroid(self) :
        ...
        centattr = np.divide(centattr, length)
        centlabel, count = stats.mode(labels)

        centroid = Point("RandPoint", centlabel, centattr)
    ...
    def medoid(self) :
        ...
        for p in self.points:
            currdist = 0
            for otherpt in self.points:
                if p != otherpt:
                    currdist += p.distance(otherpt)
            if currdist < currmin:
                medoid = p
                currmin = currdist
        return medoid
